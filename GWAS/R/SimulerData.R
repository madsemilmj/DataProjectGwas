...
#' @importFrom dplyr %>%
#' @examples
#' SimulerData(1000,100,1000,0.5,100,0.05,2)
...
SimulerData <- function(total_indiv, indiv_chunk, SNP, h, c, k, nr_workers) {
  ...
  if (does_folder_exists == 1){
    ...
    data.table::fwrite(data.table::as.data.table(MAP),
    ...
    data.table::fwrite(data.table::as.data.table(matrix(MAF,ncol = length(MAF))),
    ...
    data.table::fwrite(data.table::as.data.table(true_vals),
    ...
    data.table::fwrite(data.table::as.data.table(for_plink),
    ...
    data.table::fwrite(data.table::as.data.table(PHENO),
    ...
    future::plan(future::multisession(workers = x))
