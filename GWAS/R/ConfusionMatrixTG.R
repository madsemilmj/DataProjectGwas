#' ConfusionMatrixTG
#'
#' This function plots confusion matrix for the True Genetic Liability
...
#' @importFrom dplyr %>%
#' @examples
#' ConfusionMatrixTG(total_indiv = 1000, SNP = 1000, h = 0.5, BFC = 1)
ConfusionMatrixTG <- function(total_indiv, SNP, h, BFC){
  thr = -log10(.05/BFC)
  #Fetching relevant data
  assoc_string <- paste("./data/TG","_",format(total_indiv,scientific = F),"_",format(SNP,scientific = F),"_",h*100,"_5.qassoc", sep="")
  if (file.exists(assoc_string)){
    ...
    print("No data exists for this ConfusionMatrixCC! - Try another predefined or run our simulation")
  }
}
